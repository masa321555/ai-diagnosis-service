...
- ユーザーの現在年収・希望年収・推薦職種の年収レンジを踏まえ、ロードマップ通りに進んだ場合の現実的な想定年収をsalaryProjectionとして数値（万円単位の整数）で出力してください。currentには現在の推定年収、shortTerm/midTerm/longTermにはそれぞれの時点での到達可能な年収を入れてください。
...
【重要】各フィールドの文字数制限を厳守してください。jobTitleは20文字以内の簡潔な職種名にしてください。salaryRangeは「○○〜○○万円」の形式で15文字以内にしてください。
{
  ...
  "salaryProjection": {
    "current": 現在の想定年収（万円・整数）,
    "shortTerm": 短期目標達成時の想定年収（万円・整数）,
    "midTerm": 中期目標達成時の想定年収（万円・整数）,
    "longTerm": 長期目標達成時の想定年収（万円・整数）
  }
}`;
